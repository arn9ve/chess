...
;(function() {
    ...
    var LIGHT_POSITIONS = [
        [2, 0, 4],
        [2, 0, -4] // place at y=0 to avoid double phong reflection off the board
    ];
    var SQUARE_SIZE = 2;
    var CAMERA_POLAR_ANGLE = Math.PI / 3.5;
    var CAMERA_DISTANCE = 18.50;
    var SPARE_POSITION = {
    ...
    function parseGLBToBufferGeometry(arrayBuffer) {
        ...
        while (offset < arrayBuffer.byteLength) {
            ...
            if (chunkType === 0x4E4F534A) {
                ...
                binChunk = arrayBuffer.slice(chunkStart, chunkEnd);
            }
            offset = chunkEnd;
        }

        if (!jsonChunk || !binChunk) {
            throw new Error('Invalid GLB chunks');
        }

        var gltf = JSON.parse(decodeUTF8(jsonChunk).replace(/\u0000/g, '').trim());
        if (!gltf.meshes || !gltf.meshes[0] || !gltf.meshes[0].primitives || !gltf.meshes[0].primitives[0]) {
            throw new Error('Missing mesh primitive in GLB');
        }
        var primitive = gltf.meshes[0].primitives[0];
        var attributes = primitive.attributes || {};
        if (attributes.POSITION === undefined) {
            throw new Error('Missing POSITION accessor');
        }
        ...
        function accessorToTypedArray(accessorIndex) {
            var accessor = gltf.accessors[accessorIndex];
            var view = gltf.bufferViews[accessor.bufferView];
            var componentType = accessor.componentType;
            var componentSize = componentSizes[componentType];
            var itemSize = componentCounts[accessor.type];
            var count = accessor.count;
            var byteOffset = (view.byteOffset || 0) + (accessor.byteOffset || 0);
            var byteLength = count * itemSize * componentSize;

            var src = new Uint8Array(binChunk, byteOffset, byteLength);
            var raw = new Uint8Array(byteLength);
            raw.set(src);

            var array;
            if (componentType === 5126) {
                array = new Float32Array(raw.buffer);
            } else if (componentType === 5123) {
                array = new Uint16Array(raw.buffer);
            } else if (componentType === 5125) {
                array = new Uint32Array(raw.buffer);
            } else {
        ...
        var position = accessorToTypedArray(attributes.POSITION);
        geometry.addAttribute('position', new THREE.BufferAttribute(position.array, position.itemSize));

        if (attributes.NORMAL !== undefined) {
            var normal = accessorToTypedArray(attributes.NORMAL);
            geometry.addAttribute('normal', new THREE.BufferAttribute(normal.array, normal.itemSize));
        }

        if (attributes.TEXCOORD_0 !== undefined) {
            var uv = accessorToTypedArray(attributes.TEXCOORD_0);
            geometry.addAttribute('uv', new THREE.BufferAttribute(uv.array, uv.itemSize));
        }

        if (primitive.indices !== undefined) {
            var indices = accessorToTypedArray(primitive.indices);
    ...
    window['ChessBoard3'] = window['ChessBoard3'] || function(containerElOrId, cfg) {
            ...
            var LABEL_LOOK_AT_VECTOR = new THREE.Vector3();
            ...
            var whitePieceColor = 0xE7E6E7;
            if (cfg.hasOwnProperty('whitePieceColor') && typeof cfg.whitePieceColor === 'number') {
                whitePieceColor = cfg.whitePieceColor;
            }
            var WHITE_MATERIAL = new THREE.MeshLambertMaterial({color: new THREE.Color(whitePieceColor)});

            var whitePieceSpecular = 0xDBD9DB;
            ...
            var blackPieceColor = 0x4678EE;
            if (cfg.hasOwnProperty('blackPieceColor') && typeof cfg.blackPieceColor === 'number') {
                blackPieceColor = cfg.blackPieceColor;
            }
            var BLACK_MATERIAL = new THREE.MeshLambertMaterial({color: new THREE.Color(blackPieceColor)});

            var blackPieceSpecular = 0x60A2B8;
            ...
            var darkSquareColor = 0xD6D2F1;
            if (cfg.hasOwnProperty('darkSquareColor') && typeof cfg.darkSquareColor === 'number') {
                darkSquareColor = cfg.darkSquareColor;
            }
            var darkSquareMaterial = new THREE.MeshBasicMaterial({color: new THREE.Color(darkSquareColor)});

            var lightSquareColor= 0xEAEAEA;
            if (cfg.hasOwnProperty('lightSquareColor') && typeof cfg.lightSquareColor === 'number') {
                lightSquareColor = cfg.lightSquareColor;
            }
            var lightSquareMaterial = new THREE.MeshBasicMaterial({color: new THREE.Color(lightSquareColor)});
            /*
            ...
            */
            ...
            function expandConfig() {
                ...
                if (cfg.hasOwnProperty('pieceSet') !== true ||
                    ...
                    cfg.pieceSet = 'assets/chesspieces/hidea-glb/{piece}.glb';
                }
            ...
            function prepareScene() {
                ...
                var maxPixelRatio = 1.25;
                if (cfg.hasOwnProperty('maxPixelRatio') === true &&
                    typeof cfg.maxPixelRatio === 'number' &&
                    cfg.maxPixelRatio > 0) {
                    maxPixelRatio = cfg.maxPixelRatio;
                }
                var rendererPixelRatio = Math.min(window.devicePixelRatio || 1, maxPixelRatio);
                RENDERER = new THREE.WebGLRenderer({
                    alpha: false,
                    preserveDrawingBuffer: false,
                    antialias: rendererPixelRatio <= 1.25
                });
                ...
                var initialWidth = Math.max(1, containerEl.clientWidth);
                var initialHeight = Math.max(1, containerEl.clientHeight);
                ...
                CAMERA.aspectRatio = initialWidth / initialHeight;
                ...
                if (cfg.rotateControls || cfg.zoomControls) {
                    if (THREE.OrbitControls !== undefined) {
                        ...
                        if (cfg.rotateControls) {
                            var minPolar = Math.PI / 2 * 0.1;
                            var maxPolar = Math.PI / 2 * 0.8;
                            if (cfg.freeRotate === true) {
                                minPolar = 0.05;
                                maxPolar = Math.PI - 0.05;
                            }
                            if (typeof cfg.minPolarAngle === 'number') {
                                minPolar = cfg.minPolarAngle;
                            }
                            if (typeof cfg.maxPolarAngle === 'number') {
                                maxPolar = cfg.maxPolarAngle;
                            }
                            CAMERA_CONTROLS.minPolarAngle = minPolar;
                            CAMERA_CONTROLS.maxPolarAngle = maxPolar;
                        } else {
                        ...
                        if (cfg.zoomControls) {
                            var minDistance = 12;
                            var maxDistance = 22;
                            if (typeof cfg.minDistance === 'number') {
                                minDistance = cfg.minDistance;
                            }
                            if (typeof cfg.maxDistance === 'number') {
                                maxDistance = cfg.maxDistance;
                            }
                            CAMERA_CONTROLS.minDistance = minDistance;
                            CAMERA_CONTROLS.maxDistance = maxDistance;
            ...
            function buildBoard() {
                ...
                var hemisLight = new THREE.HemisphereLight(0xFFFFFF, 0x404040, 0.77);
            ...
            function getGeometryURL(name) {
                if (typeof cfg.pieceSet === 'function') {
                    return cfg.pieceSet(name);
                }
                if (typeof cfg.pieceSet === 'string') {
                    return cfg.pieceSet.replace("{piece}", name);
                }
                return null;
            }

            function glbFallbackURL(url) {
                var fallback = url;
                fallback = fallback.replace('/hidea-glb/', '/hidea/');
                fallback = fallback.replace(/\.glb$/i, '.json');
                return fallback;
            }

            function setPlaceholderGeometry(name) {
                var geometry = new THREE.CylinderGeometry(0.65, 0.7, 1.8, 16);
                geometry.computeBoundingBox();
                GEOMETRIES[name] = geometry;
            }

            function prepareLoadedGeometry(geometry) {
                if (!geometry) {
                    return null;
                }
                ...
                if (geometry.boundingBox && geometry.boundingBox.min && typeof geometry.translate === 'function') {
                    var yOffset = -geometry.boundingBox.min.y;
                    if (Math.abs(yOffset) > 0.0001) {
            ...
            function loadLegacyGeometry(name, url, onError) {
                ...
                if (json) {
                    try {
                        ...
                        GEOMETRIES[name] = prepareLoadedGeometry(result.geometry);
                        return;
                    } catch (e3) {
                ...
                loader.load(url, function(geometry) {
                    GEOMETRIES[name] = prepareLoadedGeometry(geometry);
                    try {
            ...
            function loadGLBGeometry(name, url, onError) {
                var loader = new THREE.XHRLoader();
                loader.setResponseType('arraybuffer');
                loader.load(url, function(arrayBuffer) {
                    try {
                        var geometry = parseGLBToBufferGeometry(arrayBuffer);
                        GEOMETRIES[name] = prepareLoadedGeometry(geometry);
                    } catch (e) {
                        if (typeof onError === 'function') {
                            onError(e);
            ...
            function loadGeometry(name) {
                var url = getGeometryURL(name);
                if (!url) {
                    setPlaceholderGeometry(name);
                    return;
                }

                if (/\.glb$/i.test(url)) {
                    loadGLBGeometry(name, url, function() {
                        loadLegacyGeometry(name, glbFallbackURL(url), function() {
                            setPlaceholderGeometry(name);
                        });
                    });
                    return;
                ...
                loadLegacyGeometry(name, url, function() {
                    setPlaceholderGeometry(name);
                });
            }
            ...
            function init() {
                ...
                function animate() {
                    ...
                    var cameraMoved = (x !== PREV_CAMERA_X
                    || y !== PREV_CAMERA_Y
                    || z !== PREV_CAMERA_Z);
                    PREV_CAMERA_X = x;
                    PREV_CAMERA_Y = y;
                    ...
                    if (RENDERER.shadowMap && RENDERER.shadowMap.enabled) {
                        var dynamicShadow = (DRAG_INFO !== null || ANIMATION_HAPPENING || cameraMoved);
                        RENDERER.shadowMap.autoUpdate = dynamicShadow;
                        if (!dynamicShadow && (RENDER_FLAG || SHADOW_NEEDS_UPDATE)) {
                    ...
                    if (RENDER_FLAG || cameraMoved) {
                        if (LABELS && LABELS.length) {
                            LABEL_LOOK_AT_VECTOR.set(100 * x, 100 * y, 100 * z);
                            for (var i = 0; i < LABELS.length; i++) {
                                LABELS[i].lookAt(LABEL_LOOK_AT_VECTOR);
                            }
                        }
                        if (x <= -8) {
